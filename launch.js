...
(async () => {
    ...
    const [launchSettings, accountsData] = await Promise.all([getLaunchSettings(), getAccountsData()]);

    // @todo: impactChatbot is legacy flag, remove when all instances migrate to "SUPPORTCENTER.VIRTUAL_ASSISTANT.ENABLED"
    if ((launchSettings.impactChatbot || launchSettings.virtualAssistant) && !window._chatBotReady) {
    ...
    async function getAccountsData() {
        // @todo: after frontend is updated we can remove adding roles prefixed by Canvas UUID and leave only SF prefix.
        // https://instructure.atlassian.net/browse/XPIPE-7547
        return (await getJSON('/api/v1/impact/data')).account;
    }
    ...
    async function handleEesySoftUser() {
        if (ENV.current_user_id === undefined || ENV.current_user_id === null) {
            ...
            if (launchSettings.supportNonAuthorizedUsers) {
                // @todo: impactChatbot is legacy flag, remove when all instances migrate to "SUPPORTCENTER.VIRTUAL_ASSISTANT.ENABLED"
                if (launchSettings.impactChatbot || launchSettings.virtualAssistant) {
    ...
    async function getRoles(launchSettings) {
        ...
        coursesWithFilteredAccounts.forEach((course) => {
            ...
            (course.enrollments || []).forEach((enrollment) => {
                ...
                if (launchSettings.hostAsRolePrefix) {
                    setRole(
                        roles,
                        `${accountsData.uuid}_${enrollment.type}`,
                        'role',
                    ...
                    setRole(
                        roles,
                        `${accountsData.salesforce_id}_${enrollment.type}`,
                        'role',
                        `${enrollment.type} - ${hostname}`,
                        hostname
                    );
                ...
                setRole(roles, `${fullAccountId}_${enrollment.type}`, 'course_role', enrollment.type, fullAccountId);
                setRole(roles, `${fullAccountId}_${enrollment.role}`, 'course_role', enrollment.role, fullAccountId);
            });
        });
        ...
        ENV.current_user_roles.forEach((role) => {
            ...
            if (launchSettings.hostAsRolePrefix) {
                setRole(roles, `${hostname}_${role}`, 'host_role', undefined, hostname);
                setRole(roles, `${accountsData.uuid}_${role}`, 'role', `${role} - ${hostname}`, hostname);
                setRole(roles, `${accountsData.salesforce_id}_${role}`, 'role', `${role} - ${hostname}`, hostname);
            }
        });

        let primaryRole = getPrimaryRole();
        setRole(roles, primaryRole.role, 'role', 'Primary ' + primaryRole.name);
        if (launchSettings.hostAsRolePrefix) {
            setRole(roles, `${hostname}_${primaryRole.role}`, 'role',
    ...
    function getNonAuthorizedRoles() {
        ...
        setRole(roles, `${hostname}_non_authorized`, 'host_role', undefined, hostname);

        setRole(roles, `${accountsData.uuid}_non_authorized`, 'role', `Non Authorized users: ${hostname}`, hostname);
        setRole(roles, `${accountsData.salesforce_id}_non_authorized`, 'role', `Non Authorized users: ${hostname}`, hostname);
